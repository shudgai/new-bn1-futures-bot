...
## 2026-09-11 還原峰谷出口並新增 KC 中軌走平禁開

- 使用者反映圖上開倉像亂開、平倉像亂平。以 09-11 20:47–04:39（台北）逐筆重建：前段 +11.61U、後段 -36.40U；25 筆已平倉 14 勝 11 敗、淨 -24.79U，其中 04:12–04:38 連續三筆硬止損 -22.85U，佔全日虧損 92%。
- 查出兩處與授權不符的實作：(1) fading_ma3_turn 於 commit 3b1df46 被改成無條件 return False，峰谷（CK 狹窄衰退＋MA3 0.10 ATR 反向）出口自當日起完全不會觸發，repo 自身 tests/test_channel_fading_exit.py 24 項因此失敗；(2) entry_trend_direction 只比較最近兩根已收線中軌的嚴格大小，沒有走平容差，V 型快通道與即時破軌入口也不看中軌方向。
- 依使用者「KC 走平行時不要開倉」與 AGENTS 授權修正：還原 fading_ma3_turn 並加 CHANNEL_FADING_MA3_EXIT_ENABLED（預設 true，停用時不平倉也不清除既有狀態）；新增 CHANNEL_FLAT_MIDDLE_RATIO（預設 0.05，最近兩根已收線中軌位移 ÷ 軌寬 低於門檻即回 KC_FLAT_MIDDLE_WAIT，多空對稱，套用趨勢／V 型／即時破軌三條入口，用軌寬相對值避免低價幣誤擋）；修正 symbol_runner 誤植的「浮盈回吐20%」日誌文字為實際的階梯鎖利。
- 證據（09-11 25 筆、本機 8006 1m K 線回算）：以位移 ÷ 軌寬 < 0.05 切分，被擋 15 筆合計 -50.84U，其餘 10 筆 +26.06U；前後段同向（前段被擋 -29.93U／非走平 +33.58U）。單日樣本，門檻仍需 30 天回放確認。
- 驗證：新專項 17 passed；全套 --continue-on-collection-errors 與 HEAD 基準逐項比對無新增失敗，28 項由失敗轉通過（含峰谷出口）。未改動出口金額、槓桿與帳戶風控。
- 未部署：本次未重啟服務，門檻與開關待使用者確認後才生效。

## 2026-09-11 多空長實體即時破軌入口

- 新增KC_LIVE_BODY_BREAKOUT_LONG/SHORT：原始開盤至最新價的同向實體至少前根已收線ATR的0.5倍，實體嚴格穿出當根同側外軌。不等待收線或MA3／CK確認，原趨勢入口保留。
- 逐報價入口與診斷改採共用決策的方向，避免相反或持平的舊CK中軌擋住已成立的即時突破；新倉、快取、掃描與一般重開共用。影線、碰軌、跳空已在軌外不當成本入口。
- 保留0.15%淨空間、末端雙向禁開、異常專用回踩、報價時效、每根限次與帳戶風控；持倉出口不變。
- 新突破專項50 passed，既有趨勢／空間／末端210 passed；指定五份回歸134 passed／73 failed，與HEAD入口及引擎對照的失敗集合完全相同。


## 2026-09-11 CK中軌趨勢入口，不看上下軌位置

- 新增entry_trend_direction，取最近兩根已收線CK中軌升降決定多空，不用當根CK，不要求外軌斜率、MA3方向或位置、動能加速與即時K同色。退出使用的ck_direction不變。
- 新倉、一般重開、掃描、逐報價及最後送單共用；KC_TREND_LONG/SHORT不再落入舊盤整穿軌確認。新趨勢快照暫時失效不新增候選鎖，既有鎖保留。
- 保留0.15%淨利空間、末端禁開、反向異常、異常專用回踩、每根限次與帳戶風控。出口不變。
- 趨勢入口86 passed，含多空、低價尺度、轨內／碰上下軌／轨外、掃描與逐報價真實送單、快取方向變化與安全攔截。前輪空間／末端124 passed。
- 指定五份回歸134 passed／73 failed；修改前入口與引擎在獨立測試程序126 passed／81 failed，完整集合比對無新增失敗、8項恢復通過。


## 2026-09-11 恢復全入口利潤空間及防止暫時不足鎖死

- 恢復既有entry_room估算：最近60根已收線內未被再次觸及的最近確認前高／前低，扣雙邊費用與雙邊滑點後達.env設定0.15%；掃描、快取送單、正常重開共用。無可靠目標不開，不捏造ATR目標。
- 空間不足於快照後獨立返回，不透過None快照新增候選失效鎖；空間恢復後可在既有每根限次內重新評估。診斷顯示profit_room，並補末端阻擋原因。
- 線上1000PEPE快照：空手、可用150U；CK中軌0.003281498961升至0.003281641917，但上軌0.003285588961降至0.003285431917，KC_DIRECTION_WAIT正確，未豁免方向風控。此為本輪快照，非所有歷史未成交的證明。
- 本次利潤空間、低價幣送單恢復、只讀診斷與末端交互測試124 passed。
- 指定五份回歸126 passed／81 failed；測試程序載入HEAD引擎作對照，失敗集合完全相同。既有出口未修改。


## 2026-09-11 末端雙向禁開與平倉後等待

- 依使用者「結束後不要再開倉，等趨勢明朗再開」：成熟且量能不足期間，多空新倉與重開均禁止；移除首根順色例外，快取及送單重新檢查。
- 已收線量能達均量1.5倍或不再符合成熟末端時解除末端阻擋，仍須CK方向與動能增強、即時MA3及既有外軌入口全部通過；平倉當根限次保留。
- 本次不新增成熟弱量立即平倉出口，持倉依現行出口管理。
- 末端專項28 passed；指定五份回歸126 passed、81 failed，與先前紀錄數量相同，本輪未重新核對隔離基準失敗集合，不宣稱全套通過。

## 2026-09-11 MA3相鄰報價小抖動誤平修正

- 核對PEPE 10:40:02、10:42:29、10:48:49、10:49:19、10:56:14與龍蝦10:36:12、10:37:07，全部為MA3_IMMEDIATE_TURN_EXIT，並非百分比回吐出口。
- 原helper比較相鄰報價MA3，只要先順向再反向即平，無真正相對已收線MA3反向或幅度門檻。接續工作區已移除引擎呼叫的修改，停用compatibility helper並清除持倉／metadata舊immediate狀態；保留有效CK衰退狹窄MA3出口與其待平、0.5U／20%回吐及緊急／硬止損。
- 專項139 passed，新增多空逐報價／掃描、舊pending與小回落仍順向續抱8項。必要五份回歸及隔離HEAD基準皆126 passed / 81 failed；精確失敗集合另見/tmp/ma3-jitter-current.txt及/tmp/ma3-jitter-baseline.txt。
- 龍蝦10:35:02有KC_LIVE_CANDLE_DIRECTION_WAIT，10:36:03–05多次送單行情重驗拒絕後成交。缺少穿軌瞬間完整快照，不宣稱已證明所有入口延遲原因，不修改入口。
- 沒有歷史逐報價重演，不宣稱修改後每筆獲利或能在最高點成交。
