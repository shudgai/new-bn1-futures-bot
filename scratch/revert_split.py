...
old_block = """                                        total_usdt = self.account.get_wallet_balance() / max(MAX_SLOTS, 1) if MAX_SLOTS > 0 else TRADE_AMOUNT_USDT
...
new_block = """                                        total_usdt = self.account.get_wallet_balance() / max(MAX_SLOTS, 1) if MAX_SLOTS > 0 else TRADE_AMOUNT_USDT
                                        await self.account.open_position(
                                            symbol=symbol,
                                            side=cr_signal,
                                            price=live_price,
