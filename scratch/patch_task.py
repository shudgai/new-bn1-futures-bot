...
patch = """
- `[ ]` 6. **防追空防禦模組 (Anti-Chase V5.1)**
    - `[ ]` `outer_strategy.py` 實作邊界過濾 (`Price < kc_lower` 拒絕)
    - `[ ]` `outer_strategy.py` 實作乖離過濾 (`Distance > 1.5 ATR` 拒絕)
    - `[ ]` `outer_strategy.py` 實作影線過濾 (`Lower_Wick / Body > 1.2` 拒絕)
    - `[ ]` `outer_strategy.py` 確保在回抽中軌時才啟動 `velocity_slowdown` 進場
"""
