...
async def get_status(response: Response):
    ...
    return {
        ...
        "strategy": f"盤中峰谷與即時CK外側入口：CK方向未明一律不開。方向採最近兩根已收線CK中軌嚴格升降，持倉側外軌不得逆向；持平、不一致或資料無效都等待。多單另須即時MA3高於上一根已收線MA3，空單須低於；MA3反向、持平或無效均不開。順CK的峰谷報價轉向可評估，或最新價嚴格在同側CK外軌外即評估，不等兩根K或六根延續確認，不要求MA3／MA15交叉。新倉、快取、正常與異常重開、反手新倉均重驗CK與MA3；異常平倉專用回踩、正常平倉當根禁開及每根限次保留。每次新倉均檢查未突破已確認前高／前低，扣費滑點後淨空間須達0.15%，無有效目標或空間不足不開。硬止損與CK反向先平舊倉優先，新倉須通過當前條件。預估淨利1USDT啟動20%回吐保護，保留淨利底線及更有利保護價；啟動後不另走MA3、異常K或瀑布出口。未啟動時保留大瀑布與雙已收線反向異常；MA3進場後先順向，相對上一根已收線的反向幅度與峰谷回退幅度都須達固定0.10 ATR才平，小弧度不平。反向異常進場攔截及所有帳戶風控保留（{len(DEFAULT_SYMBOLS)}幣）",
