...
async def get_status(response: Response):
    ...
    return {
        ...
        "strategy": "Sniper Strategy (嚴格過濾進場 + 極致鈍感持倉 + 極速動能防禦)：\n[進場] 實體強勁突破(>=50%) KC中軌 + 通道明確擴張傾斜 + 突破距離超過 0.5 ATR + 預期獲利空間 >= 1.5 ATR (盈虧比 > 3:1) + 最新即時價未回補假突破。\n[出場] 純機械式雙軌獨立掛單：1. 0.5 ATR 固定防禦線 (絕不隨波逐流上移)。2. 1.0 ATR 階梯限價鎖利單。\n[緊急避險] 最高優先級流動性獵殺防禦：遭遇大陰/大陽線實體吞沒、單棒暴跌/暴漲 > 1.5 ATR、或兩棒內極速回撤 > 1.0 ATR 時，無視一切條件立即市價逃離。",
