...
async def get_status(response: Response):
    ...
    return {
        ...
        "strategy": f"雙入口：有效上下軌突破可進場；未重新破軌的延續須最近6根已收線K持續同向推進，排除反覆穿越中軌、CK轉向及實體大量重疊的波浪。CK外軌突破與延續開倉：最近兩根已收線同向實體均收在同側外軌外，多單兩綠、空單兩紅，每根實體至少20%；不要求第一根重新穿軌；最新價仍在同側外軌外。MA3／MA15／KC須順向，即時MA3多單嚴格上升、空單嚴格下降，持平或反向不開。新倉、重開與送單前統一重驗；保留同根回調、異常等待、每根限次、末端及帳戶風控。預估淨利1USDT啟動20%回吐保護，保留淨利底線及既有更有利保護價；啟動後MA3、異常K與瀑布不另行平倉。未啟動保護時保留MA3轉彎、大瀑布及雙已收線反向異常出口，單根反向異常不平倉；送單前反向異常仍禁止開倉，手動及帳戶硬停損保留（{len(DEFAULT_SYMBOLS)}幣）",
