...
async def get_status(response: Response):
    ...
    return {
        ...
        "strategy": f"盤中峰谷與即時CK外側入口：CK方向未明一律不開。方向採最近兩根已收線CK中軌嚴格升降，持倉側外軌不得逆向；持平、不一致或資料無效都等待。多單另須即時MA3高於上一根已收線MA3，空單須低於；MA3反向、持平或無效均不開。順CK的峰谷報價轉向可評估，或最新價嚴格在同側CK外軌外即評估，不等兩根K或六根延續確認，不要求MA3／MA15交叉。新倉、快取、正常與異常重開、反手新倉均重驗CK與MA3；異常平倉專用回踩、正常平倉當根禁開及每根限次保留。每次新倉均檢查未突破已確認前高／前低，扣費滑點後淨空間須達0.15%，無有效目標或空間不足不開。帳戶硬止損保留；CK確認反向只平倉，不立即反手，下一根重新評估一般入口。取消MA3轉向平倉與百分比浮盈回吐。扣雙邊費用及預估滑點後淨利達4USDT鎖2USDT、達6鎖4，之後每2USDT上移一階，回落至鎖定線全平。大瀑布與連續兩根已收線反向異常K在鎖利前後均可緊急平倉。反向異常進場攔截及所有帳戶風控保留（{len(DEFAULT_SYMBOLS)}幣）",
