...
async def get_status(response: Response):
    ...
    return {
        ...
        "strategy": f"盤中峰谷與即時CK外側入口：CK方向未明一律不開。方向採最近兩根已收線CK中軌嚴格升降，持倉側外軌不得逆向；持平、不一致或資料無效都等待。多單另須即時MA3高於上一根已收線MA3，空單須低於；MA3反向、持平或無效均不開。順CK的峰谷報價轉向可評估，或最新價嚴格在同側CK外軌外即評估，不等兩根K或六根延續確認，不要求MA3／MA15交叉。一般新倉、快取與同向重開均重驗CK與MA3；成功平倉的直接反手例外不等方向訊號。異常平倉同向重開保留專用回踩；下一根起若CK與即時MA3確認相反方向、無衰退及反向異常，匹配成功平倉後解除舊方向票據，重新驗證一般入口。正常平倉當根禁開及每根限次保留。最近4根已收線CK中軌的3次順向位移連續縮小時暫停一般新倉與同向重開，動能恢復重新評估；不觸發持倉平倉。已取消所有新倉與重開的獲利空間篩選，不以0.15%門檻或缺少前高／前低目標攔截。帳戶硬止損保留；固定階梯或CK平倉成功後，以匹配成交票據立即評估反向新倉，不等CK、MA3或外軌，保留報價、異常與帳戶風控；票據僅平倉當根有效且只能成交一次。反手倉等CK首次同向後才啟用CK反向出口，之前仍由固定階梯與緊急止損管理。大瀑布／雙異常、硬止損及手動平倉不立即反手。取消MA3轉向平倉與百分比浮盈回吐。扣雙邊費用及預估滑點後淨利達4USDT鎖2USDT、達6鎖4，之後每2USDT上移一階，回落至鎖定線全平。大瀑布與連續兩根已收線反向異常K在鎖利前後均可緊急平倉。反向異常進場攔截及所有帳戶風控保留（{len(DEFAULT_SYMBOLS)}幣）",
