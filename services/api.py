...
from core.services.rule_registry import active_entry_rule_lines, active_exit_rule_lines
...
async def get_status(response: Response):
    ...
    return {
        ...
        "strategy": "\n".join(active_entry_rule_lines() + active_exit_rule_lines()),
...
async def _load_klines(symbol: str, timeframe: str, limit: int, include_live: bool):
    ...
    try:
        ...
        if timeframe == "1m" and len(indicators) >= 4:
            pivot_markers = build_pivot_markers(indicators)
        trade_markers = {}
