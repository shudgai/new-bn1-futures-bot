...
async def get_status(response: Response):
    ...
    return {
        ...
        "strategy": f"CK外軌突破與延續開倉：最近兩根已收線同向實體均收在同側外軌外，多單兩綠、空單兩紅，每根實體至少20%；不要求第一根重新穿軌；最新價仍在同側外軌外。MA3／MA15／KC須順向，即時MA3多單嚴格上升、空單嚴格下降，持平或反向不開。新倉、重開與送單前統一重驗；保留同根回調、異常等待、每根限次、末端及帳戶風控。策略自動平倉只用最高浮盈回吐20%，預估淨利1USDT啟動保護；反向異常K、瀑布與MA3轉彎不單獨平倉，手動及帳戶硬停損保留（{len(DEFAULT_SYMBOLS)}幣）",
