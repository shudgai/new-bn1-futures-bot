...
async def get_status(response: Response):
    ...
    return {
        ...
        "strategy": f"上下外軌破軌唯一入口：第一根已收線同向有效實體穿出CK外軌，第二根同色有效實體收在同側外軌外，每根實體至少全長20%；送單最新價須嚴格在同側軌外。一般新倉、重開與快取送單共用，不使用盤中峰谷、單純軌外追入或直接反手。保留最近兩根已收線CK中軌方向、持倉側外軌不逆向、即時MA3同向、CK衰退、反向異常與帳戶風控。扣雙邊費用與預估平倉滑點後，淨浮盈達1USDT啟動保護，最高淨浮盈回吐20%全平；已有更有利保護線與待平重試保留。平倉後不反手，下一根起重新驗證破軌入口；異常同向重開仍須回踩。CK最近四根已收線中軌的三次順向位移連續縮小時，進場後觀察MA3順向峰谷再明顯反向0.10ATR可平倉；不反手，等待平倉後新的兩根破軌確認。未衰退時MA3轉向不平倉，1U／20%鎖利獨立有效。保留大瀑布、雙反向異常K與帳戶硬止損。不恢復獲利空間攔截。每根限次與送單重驗保留（{len(DEFAULT_SYMBOLS)}幣）",
