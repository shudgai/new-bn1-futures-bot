...
def test_aligned_trend_can_enter_without_breakout_body_confirmation():
    df = _generate_macro_frame("UP", 70)
...
def test_single_bar_holds_and_two_abnormal_bars_exit():
    """2026-09-11: single-bar waterfall exits removed; two closed bodies need 1.0 ATR each."""
    df = _generate_macro_frame("UP", 70)
    df.loc[67, ["open", "close"]] = [100.0, 100.6]
    df.loc[68, ["open", "close"]] = [100.6, 101.2]
    df.loc[69, ["open", "close"]] = [100.0, 101.5]
    assert TradingEngine._channel_adverse_exit_reason(df, "SHORT", 101.5, 1.0) is None
    df.loc[67, ["open", "close"]] = [100.0, 101.2]
    df.loc[68, ["open", "close"]] = [101.2, 102.4]
    assert TradingEngine._channel_adverse_exit_reason(df, "SHORT", 100.1, 1.0) == "EMERGENCY_EXIT_2_CANDLE_ADVERSE"
    df.loc[67, ["open", "close"]] = [100.0, 100.9]
    df.loc[68, ["open", "close"]] = [100.9, 101.8]
...
def test_outer_reentry_spike_breakout_is_not_traded():
    ...
    res = TradingEngine._channel_swing_action(df, 107.6, None, outer_entry_only=True)
    assert res["action"] == "WAIT"
    assert res["reason"] == "KC_OUTSIDE_WAIT_NEXT_CANDLE"


def test_outside_long_signal_waits_after_opposite_closed_body():
    """A red closed candle cannot confirm a long entry after a green spike."""
    df = _generate_macro_frame("UP", 70)
    ...
    assert result == {"action": "WAIT", "side": None, "reason": "KC_SURGE_WAIT_TROUGH"}


def test_live_price_above_upper_rail_waits_without_valid_closed_body():
    df = _generate_macro_frame("UP", 70)
    df.loc[68, ["close", "kc_upper"]] = [107.2, 107.0]
    df.loc[68, "high"] = 107.3
...
def test_live_price_below_lower_rail_waits_without_valid_closed_bodies():
    df = _generate_macro_frame('DOWN', 70)
...
def test_tail_entry_blocked_after_long_one_way_run():
    ...
    # Fail closed: too few bars to measure the run must refuse the entry.
    short = pd.DataFrame({"kc_middle": [100.0, 100.01, 100.02]})
    assert channel_tail_entry_blocked(short, "LONG") is True
...
def _overheat_frame(side, body_atr):
    ...
    rows.append(dict(open=open_price, high=max(open_price, price) + 0.01,
                     ...
                     ma3=last, ma15=last, ema_20=mids[-1], atr=atr, timestamp=14.0))
