...
import pytest
from core.services.strategies.outer_strategy import sustained_trend_ready, aligned_entry, aligned_entry_ready, outside_reentry
...
def trend(side):
    ...
    for n,i in enumerate(f.index[-7:-1]):
        f.loc[i,['open','close','kc_middle']]=[101+n*.3,101.25+n*.3,97.4+n*.8]
