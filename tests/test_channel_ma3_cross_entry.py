...
import pytest
from core.services.strategies.outer_strategy import aligned_entry, ma3_outer_cross_ready, outside_reentry
...
def test_shared_crossing_rule(side,case):
    ...
    # already_outside 的框架兩根已收線都是十字（非實體K）→ 連延續也不成立。
    reentry_expected = case == 'valid'
