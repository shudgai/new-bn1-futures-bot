...
class TradingEngine:
    ...
    async def _monitor_pullback_candidates(self, now: float) -> None:
        # Retired candidates must never reach the old maker-entry route.
    ...
    async def _validate_pending_limit_orders(self, now: float) -> None:
        # Channel Swing uses market entries only; cancel rather than fill old limits.
        for symbol in list(self.account.pending_limit_orders):
            await self.account.cancel_pending_limit(symbol, "舊掛單策略停用，等待新外軌訊號")
        return
        """先驗證再查成交；最大限度縮小失效訊號被舊掛單接住的時間窗。"""
        for symbol, order in list(self.account.pending_limit_orders.items()):
            if (order.get("entry_context") or {}).get("entry_mode") != "CHANNEL_SWING":
                await self.account.cancel_pending_limit(symbol, "舊交易策略已移除，取消舊掛單")
        if any((order.get("entry_context") or {}).get("entry_mode") != "CHANNEL_SWING"
               for order in self.account.pending_limit_orders.values()):
            return
        daily_halt, _ = self.account.daily_loss_limit_hit()
        daily_halt = daily_halt or self._market_crash_entries_paused(now)
        if daily_halt:
    ...
    def _channel_long_profit_room(frame, price):
        return TradingEngine._channel_profit_room(frame, price, "LONG")
    ...
    def _channel_peak_exit_reentry_blocked(
        ...
        if peak_exit_info.get("require_new_closed_break"):
            try:
                ...
                breakout_id = breakout.get("timestamp", frame.index[-3])
    ...
    def _channel_peak_reversal_action(
        ...
        if peak_exit_info and peak_exit_info.get("require_new_closed_break"):
            return {"action": "WAIT", "side": None, "reason": "WAIT_NEW_CLOSED_BREAK_AFTER_EXIT"}
    ...
    def _channel_adverse_exit_reason(frame: pd.DataFrame, side: str, price: float, atr: float) -> str | None:
        """Exit on a 2x abnormal body or two individually abnormal adverse bars."""
    ...
    @staticmethod
    def _channel_ma3_outside(row: pd.Series, side: str) -> bool:
        try:
            ma3, upper, lower = (float(row[key]) for key in ("ma3", "kc_upper", "kc_lower"))
            return bool(
                all(math.isfinite(value) and value > 0 for value in (ma3, upper, lower))
                and lower < upper
                and ((side == "LONG" and ma3 >= upper) or (side == "SHORT" and ma3 <= lower))
            )
        except (TypeError, ValueError, KeyError):
            return False

    @staticmethod
    def _channel_outer_half_space_hold(previous: pd.Series, current: pd.Series, side: str) -> bool:
        """Hold ordinary turns while MA3 stays outside and rail-to-MA15 room is >= half KC."""
        if not all(TradingEngine._channel_ma3_outside(row, side) for row in (previous, current)):
            return False
        try:
            upper, lower, ma15 = (float(current[key]) for key in ("kc_upper", "kc_lower", "ma15"))
            gap = upper - ma15 if side == "LONG" else ma15 - lower
            return bool(math.isfinite(ma15) and ma15 > 0 and gap >= (upper - lower) * 0.5)
        except (TypeError, ValueError, KeyError):
            return False
    ...
    @staticmethod
    def _channel_ma15_convergence_is_gradual(
        frame: pd.DataFrame, rail: str, atr: float,
    ) -> bool:
        """Accept only multi-bar rail/MA15 convergence, never a one-bar jump."""
        required = {rail, "ma15"}
        if frame is None or len(frame) < 6 or not required.issubset(frame.columns):
            return False
        try:
            closed = frame.iloc[-6:-1]
            gaps = [
                abs(float(row[rail]) - float(row["ma15"]))
                for _, row in closed.iterrows()
            ]
            if not all(math.isfinite(gap) for gap in gaps):
                return False
            atr_value = max(float(atr), 1e-12)
        except (TypeError, ValueError, KeyError):
            return False
        reductions = [previous - current for previous, current in zip(gaps, gaps[1:])]
        return bool(
            all(reduction > 0.0 for reduction in reductions)
            and gaps[0] - gaps[-1] >= atr_value * 0.50
            and max(reductions) <= atr_value * 0.50
        )
    ...
    def _channel_trend_exit_reason(frame: pd.DataFrame, side: str, entry_width: float = 0.0) -> str | None:
        ...
        try:
            ...
            widths = pd.to_numeric(closed["kc_upper"], errors="coerce") - pd.to_numeric(closed["kc_lower"], errors="coerce")
            valid_widths = widths.tail(60)
            valid_widths = valid_widths[valid_widths.map(lambda value: math.isfinite(value) and value > 0)]
            width = float(widths.iloc[-1])
            reference = max(float(valid_widths.max()), float(entry_width or 0.0))
            ma3_before, ma3_now, ma15 = float(previous["ma3"]), float(current["ma3"]), float(current["ma15"])
            upper, lower = float(current["kc_upper"]), float(current["kc_lower"])
    ...
    @staticmethod
    def _channel_position_path(frame, side, opened_at, state=None):
        """Remember only this position's post-entry outside -> inside sequence."""
        state = state if state is not None else {}
        identity = (side, opened_at)
        if tuple(state.get("identity", ())) != identity:
            state.clear()
            state["identity"] = identity
        if not opened_at or "timestamp" not in frame:
            return frame.iloc[0:0], False
        recent = frame[pd.to_numeric(frame["timestamp"], errors="coerce") >= float(opened_at) * 1000]
        for _, row in recent.iterrows():
            bar = float(row["timestamp"])
            if bar < state.get("last_bar", 0):
                continue
            if TradingEngine._channel_ma3_outside(row, side):
                state.setdefault("outside_bar", bar)
            elif (state.get("outside_bar") is not None and bar > state["outside_bar"]
                  and float(row["kc_lower"]) < float(row["ma3"]) < float(row["kc_upper"])):
                state["reentered"] = True
            state["last_bar"] = bar
        return recent, bool(state.get("reentered"))

    @staticmethod
    def _channel_impulse_first_turn(frame, side, price):
        """Exit the first opposite body after an outer-rail surge, regardless of size."""
        direction = 1 if side == "LONG" else -1
        for offset in (-2, -1):
            pos = len(frame) + offset
            if pos < 1:
                continue
            row, previous = frame.iloc[pos], frame.iloc[pos - 1]
            close = price if offset == -1 else float(row["close"])
            threshold = float(previous.get("atr", 0)) * RAPID_PIVOT_IMMEDIATE_REVERSE_BODY_ATR
            if not math.isfinite(threshold) or threshold <= 0:
                continue
            rail = float(previous["kc_upper"] if side == "LONG" else previous["kc_lower"])
            if (direction * (float(previous["close"]) - rail) <= 0
                    or direction * (float(row["open"]) - close) <= 0):
                continue
            # Small same-direction candles may extend the surge to its eventual peak.
            # They never add up to an impulse, and an earlier opposite body ends it.
            next_long = False
            for index in range(pos - 1, -1, -1):
                impulse = frame.iloc[index]
                body = direction * (float(impulse["close"]) - float(impulse["open"]))
                limit = float(impulse.get("atr", 0)) * RAPID_PIVOT_IMMEDIATE_REVERSE_BODY_ATR
                if not math.isfinite(body) or body < 0 or not math.isfinite(limit) or limit <= 0:
                    break
                is_long = body >= limit
                if body >= limit * 2 or (is_long and next_long):
                    return True
                next_long = is_long
        return False
    ...
    @staticmethod
    def _channel_closed_waves_falling(frame: pd.DataFrame, side: str = "LONG") -> bool:
        ...
        try:
            ...
            sign = 1 if side == "LONG" else -1
            values = (sign * middle.iloc[-3:]).tolist()
            highs = (sign * closed["high" if sign == 1 else "low"].astype(float)).tolist()
            lows = (sign * closed["low" if sign == 1 else "high"].astype(float)).tolist()
    ...
    def _channel_swing_action(
        ...
        upper_break_confirmed = bo_open <= bo_upper < bo_close and cf_close > cf_open and cf_close > cf_upper and cf_is_solid and bo_is_solid
        ...
        upper_trend_entry = bo_close > bo_open and bo_is_solid and bo_close > bo_upper and cf_close > cf_open and cf_close > cf_upper and cf_is_solid
        ...
        if held in ("LONG", "SHORT"):
            ...
            # Both sides exit immediately on a live adverse long body.
            ...
            if valid_live and live_width > 0 and adverse_body > 0 and long_body:
                reason = ("KC_SHORT_LIVE_GREEN_LONG_EXIT" if held == "SHORT"
            ...
            # Other candle patterns retain the existing channel path requirements.
            if not path_ready:
            ...
            if held == "SHORT" and upper_break_confirmed and live_price > float(current_live["kc_upper"]):
            ...
            elif "kc_middle" in current_live and pd.notna(current_live["kc_middle"]):
                middle_now = float(current_live["kc_middle"])
            else:
        ...
        history = pd.to_numeric(frame["ma15"].iloc[-4:-1], errors="coerce")
        trend = 0.0
        if len(history) == 3 and all(math.isfinite(value) and value > 0 for value in history):
        ...
        if not held:
            ...
            live_upper = float(current_live["kc_upper"])
            live_lower = float(current_live["kc_lower"])
            if live_price > live_upper and not long_entry_ready(frame, live_price):
                return {**wait, "reason": "KC_LONG_BODY_OR_SLOPE_WAIT"}
            if live_price < live_lower and not directional_entry_ready(frame, live_price, "SHORT"):
                return {**wait, "reason": "KC_SHORT_BODY_OR_SLOPE_WAIT"}
            if live_price < live_lower and TradingEngine._channel_closed_waves_falling(frame, "SHORT"):
                return {**wait, "reason": "KC_RISING_WAVES_BLOCK_SHORT"}
            ...
            # 順勢進場 (已經在外軌外，趨勢延續)：需受最近三根已收線 MA15 方向限制
            if upper_trend_entry and trend > 0 and live_price > live_upper:
                return {"action": "ENTER", "side": "LONG", "reason": "KC_UPPER_TREND_ENTRY"}
            if lower_trend_entry and trend < 0 and live_price < live_lower:
    ...
    async def _execute_confirmed_channel_break(self, symbol, frame, price, side, daily_halt=False):
        ...
        async with lock:
            ...
            if daily_halt:
                ...
                return False
            latest = frame.iloc[-2]
            confirmation_label = "MA15 aligned confirmed price pivot" if decision["reason"] in PIVOT_CODES else "two closed candles"
            signal = {
    ...
    def _profit_reentry_ready(self, symbol, ticket, frame, price):
        ...
        if result == "ready" and self._channel_swing_action(frame, price, outer_entry_only=True).get("side") != ticket["side"]:
            result = "wait"
        if result == "end":
            ...
            self._channel_swing_peak_exit_info[symbol] = {
                "side": ticket["side"], "exit_bar_id": frame.iloc[-1].get("timestamp", frame.index[-1]),
    ...
    async def _try_profit_reentry(self, symbol, frame, price, daily_halt):
        ...
        if not ticket or symbol in self.account.positions:
            return
        # A crash after close is recoverable only with its matching close trade.
        ...
        try:
            ...
            if existing_pos:
                ...
                if previous_protection != existing_pos.get("channel_profit_protection"):
                    self.account.save_state()
                # Keep the ordinary middle rule disabled; unarmed positions have the fallback below.
                ...
                if (existing_pos.get("channel_pivot_middle_exit_pending")
                    ...
                    channel_action = {"action": "EXIT", "side": None,
                                      "reason": f"KC_{existing_pos['side']}_UNARMED_MIDDLE_EXIT"}
                # Unarmed positions must not remain trapped waiting for profit.
                if (existing_pos.get("side") in ("LONG", "SHORT")
                        and not existing_pos.get("channel_profit_protection", {}).get("armed")
                        and channel_action.get("action") not in {"EXIT", "REVERSE"}):
                    row = channel_df.iloc[-1]
                    middle = row.get("ema_20", float("nan"))
                    if pd.isna(middle):
                        middle = row.get("kc_middle", float("nan"))
                    if pd.isna(middle):
                        middle = (float(row["kc_upper"]) + float(row["kc_lower"])) / 2
                    middle = float(middle)
                    pivot_state_before = (
                        existing_pos.get("channel_pivot_middle_reached"),
                        existing_pos.get("channel_pivot_middle_exit_pending"),
                    )
                    middle_ready = pivot_middle_exit(existing_pos, channel_price, middle)
                    if pivot_state_before != (
                        existing_pos.get("channel_pivot_middle_reached"),
                        existing_pos.get("channel_pivot_middle_exit_pending"),
                    ):
                        meta = self.account.position_meta.setdefault(symbol, {})
                        for key in ("channel_pivot_middle_reached", "channel_pivot_middle_exit_pending"):
                            meta[key] = existing_pos.get(key, False)
                        self.account.save_state()
                    if (math.isfinite(middle) and middle > 0
                            and math.isfinite(channel_price) and channel_price > 0
                            and middle_ready
                            and (existing_pos.get("channel_pivot_middle_exit_pending")
                                 or (1 if existing_pos["side"] == "LONG" else -1) * (channel_price - middle) <= 0)):
                        channel_action = {"action": "EXIT", "side": None,
                                          "reason": f"KC_{existing_pos['side']}_UNARMED_MIDDLE_EXIT"}
            ...
            if not existing_pos and pending_side in ("LONG", "SHORT"):
                ...
                if retry_bar == (pending_side, self._channel_candidate_bar_id(channel_df)):
                    retry_decision = self._channel_swing_action(
            ...
            if existing_pos and channel_action.get("action") == "EXIT":
                ...
                return signal_progress, detected_candidates
            # HOLD/WAIT must not fall through to legacy MA exits or live retries.
            self._record_channel_signal_event(symbol, channel_action.get("reason"), channel_df)
            return signal_progress, detected_candidates
