...
from core.channel_outer_entry import aligned_entry, aligned_entry_ready, live_adverse_entry_safe, ck_direction
...
class TradingEngine:
    ...
    def _live_pivot_ready(self, symbol, frame, price, side):
        ...
        if pivot is None:
            pivot = self._channel_live_pivots = LivePivot()
        if symbol in self.account.positions or side not in ('LONG', 'SHORT') or ck_direction(frame) != side:
            pivot.reset(symbol)
            return False
        ...
        if (not math.isfinite(quoted) or not 0 <= time.time() - quoted <= 5
                or math.floor(time.time() / 60) != math.floor(quoted / 60)):
            pivot.reset(symbol)
            return False
    ...
    async def _place_structured_entry_locked(
        ...
        if entry_mode == "CHANNEL_SWING":
            ...
            fresh_snapshot = None if signal.get("profit_reentry_token") or signal.get("live_pivot") else channel_snapshot
        ...
        if self._channel_candle_entry_blocked(symbol) and not self._ck_reverse_order_authorized(symbol, signal):
            ...
            return False
        if entry_mode == "CHANNEL_SWING":
            if live_pivot and self._ck_reverse_new_leg_halted():
                return False
    ...
    def _channel_peak_reversal_action(
        ...
        try:
            ...
            ma3_1 = float(first.get("ma3", 0.0))
            ma3_2 = float(second.get("ma3", 0.0))
            live_ma3 = float(live_row.get("ma3", 0.0)) if live_row is not None else ma3_2
            ...
            if exited_side == "LONG":
                confirmed = (
                    ...
                    and ma3_1 > ma3_2 > live_ma3  # MA3 strictly descending
                )
                if confirmed:
                ...
                confirmed = (
                    ...
                    and float(live_price) >= float(second["close"])
                    and ma3_1 < ma3_2 < live_ma3  # MA3 strictly ascending
                )
                if confirmed:
    ...
    @staticmethod
    def _channel_live_ma3_turn_exit(position: dict, frame: pd.DataFrame, price: float) -> bool:
        ...
        try:
            ...
            # A turn on the entry candle needs an actual post-entry observation.
    ...
    async def _execute_confirmed_channel_break(self, symbol, frame, price, side, daily_halt=False):
        ...
        # 嚴格驗證「所有新倉與重開必須有最近兩根同向且實體達 20% 的已收線K棒」
        from core.channel_outer_entry import two_closed_bodies_ready
    ...
    async def _try_ck_reverse(self, symbol, frame, price, daily_halt):
        ...
        async with locks.setdefault(symbol, asyncio.Lock()):
            ...
            if pending and ticket.get('opened_at') != position.get('open_timestamp'):
                ...
                self.account.save_state()
                pending = False
            if not pending and (side is None or side == position.get('side')):
                return False
            ...
            if not pending:
                ...
                ticket = dict(mode='ck_reverse', phase='closing', side=side,
                              ...
                              close_reason='Channel Swing CK_REVERSE ' + token,
    ...
    async def _try_profit_reentry_locked(self, symbol, frame, price, daily_halt):
        ...
        signal = {"live_pivot": live_pivot, "side": ticket["side"], "score": 100, "entry_mode": "CHANNEL_SWING",
    ...
    async def _process_single_symbol_locked(self, symbol, now_time, btc_1m_turn, daily_halt,
        ...
        try:
            ...
            if exit_only and symbol not in self.account.positions:
                return signal_progress, detected_candidates
            channel_df = exit_frame
            if channel_df is None:
                channel_df = await self.fetch_klines(symbol, timeframe="1m", limit=200, keep_live=True)
                if not channel_df.empty:
                    channel_df = self.strategy.compute_indicators(channel_df.copy())
            if not channel_df.empty:
                cache = getattr(self, "_channel_exit_frames", None)
                if cache is None:
                    cache = self._channel_exit_frames = {}
            ...
            if exit_only and not existing_pos:
                return signal_progress, detected_candidates
            if existing_pos:
                if await enforce_hard_stop(self.account, symbol, channel_price):
                    return signal_progress, detected_candidates
                if await self._try_ck_reverse(symbol, channel_df, channel_price, daily_halt):
                    return signal_progress, detected_candidates
            if not existing_pos:
                if await self._try_live_pivot_entry(symbol, channel_df, channel_price, daily_halt):
                    return signal_progress, detected_candidates
