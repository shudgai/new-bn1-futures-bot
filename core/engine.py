from core.channel_fading_exit import fading_ma3_turn, next_breakout_ready, STATE_KEY as FADING_STATE_KEY, EXIT_REASON as FADING_EXIT_REASON, IMMEDIATE_EXIT_REASON
...
class TradingEngine:
    ...
    async def _place_structured_entry_locked(
        ...
        if entry_mode == "CHANNEL_SWING":
            ...
            if fresh_snapshot is None:
                ...
                if watcher is not None:
                    watcher.reset(symbol)
                if invalid_candidate_key is not None and not live_outer_entry and not signal.get("live_pivot") and not signal.get("live_outer"):
        ...
        if self._channel_candle_entry_blocked(symbol) and not self._ck_reverse_order_authorized(symbol, signal):
            ...
            return False
        if entry_mode == "CHANNEL_SWING":
            if (live_pivot or signal.get("live_outer")) and self._ck_reverse_new_leg_halted():
                return False
    ...
    async def _process_single_symbol_locked(self, symbol, now_time, btc_1m_turn, daily_halt,
        ...
        try:
            ...
            if (
                ...
                and channel_action.get("reason") not in LIVE_OUTER_CODES
            ...
            if existing_pos:
                ...
                if channel_action.get("action") in {"EXIT", "REVERSE"}:
                    ...
                    self.account.save_state()
                    self.account.log(f"🛡️ [獲利保護] {symbol} 浮盈回吐20% 淨利峰值={profit['peak_net']:.4f} 鎖定淨利={profit['locked_net']:.2f} 保護價={profit['stop_price']:.10g} 預估淨利={profit['net_pnl']:.4f}", "INFO")
                    closed = await self.account.close_position(
