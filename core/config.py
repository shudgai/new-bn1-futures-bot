...
# Binance API 用量監控：每分鐘請求權重上限與警示門檻（讀回應標頭 X-MBX-USED-WEIGHT-1M）。
API_WEIGHT_LIMIT_PER_MIN = max(1, int(float(os.getenv("API_WEIGHT_LIMIT_PER_MIN", "2400"))))
API_WEIGHT_WARN_PCT = max(1.0, float(os.getenv("API_WEIGHT_WARN_PCT", "70")))
...
# 2026-09-14 使用者：特例K「早點入袋為安」——啟動門檻 2U、回吐 1U（一般進場仍用 4U/2U）。
CHANNEL_SWING_PROFIT_LADDER_ARM_SPECIAL_K_USDT = max(
    0.0, float(os.getenv("CHANNEL_SWING_PROFIT_LADDER_ARM_SPECIAL_K_USDT", "2.0"))
)
CHANNEL_SWING_PROFIT_LADDER_LOCK_OFFSET_SPECIAL_K_USDT = max(
    0.1, float(os.getenv("CHANNEL_SWING_PROFIT_LADDER_LOCK_OFFSET_SPECIAL_K_USDT", "1.0"))
)
...
# 1h 大週期趨勢過濾：1h SuperTrend 方向與進場方向不一致就不開新倉（快取未知時不擋）。
CHANNEL_1H_TREND_FILTER_ENABLED = os.getenv(
    "CHANNEL_1H_TREND_FILTER_ENABLED", "true"
).lower() == "true"
...
TRAILING_TIER2_TRIGGER_ATR_MULT = float(os.getenv("TRAILING_TIER2_TRIGGER_ATR_MULT", "2.5"))
TRAILING_TIER3_TRIGGER_ATR_MULT = float(os.getenv("TRAILING_TIER3_TRIGGER_ATR_MULT", "4.0"))
TRAILING_TIER2_LOCK_ATR_MULT = float(os.getenv("TRAILING_TIER2_LOCK_ATR_MULT", "1.2"))
...
MIN_SL_DISTANCE_PCT = float(os.getenv("MIN_SL_DISTANCE_PCT", "0.0020"))
