...
SUPPORT_PULLBACK_RSI_LONG_MAX = float(os.getenv("SUPPORT_PULLBACK_RSI_LONG_MAX", "68"))
SUPPORT_PULLBACK_RSI_SHORT_MIN = float(os.getenv("SUPPORT_PULLBACK_RSI_SHORT_MIN", "32"))
...
SUPPORT_PULLBACK_MAX_VOLUME_RATIO = float(os.getenv("SUPPORT_PULLBACK_MAX_VOLUME_RATIO", "1.00"))
...
# BOUNCE 是短程反彈單，使用獨立的淨風報比門檻；避免沿用趨勢單 2:1
# 導致幾乎所有合理短程掛單都被拒絕。
...
BOUNCE_NO_FOLLOW_THROUGH_SEC = max(0.0, float(os.getenv("BOUNCE_NO_FOLLOW_THROUGH_SEC", "2700")))
BOUNCE_NO_FOLLOW_THROUGH_MIN_MFE_PCT = max(
    0.0, float(os.getenv("BOUNCE_NO_FOLLOW_THROUGH_MIN_MFE_PCT", "0.002"))
)
