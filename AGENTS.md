...
- 最新出場分流授權：圖 1／圖 3 停用一般中軌平倉，改用浮盈保護（未達 1 USDT 啟動門檻也不走一般中軌出口）；圖 2 未啟動保護時沿用一般中軌平倉。已啟動保護不撤除，確認反手、緊急停損保留。獲利平倉後原中軌訊號不取消外軌重開；重開新倉重新分類，不繼承前倉峰值、保護價或 10% 限制。
- 圖 1 使用最高未扣費浮盈回吐 30%；圖 2 沿用一般通道出場，不新啟動浮盈保護；圖 3 堆疊式同向推進使用浮盈保護，遇即時反色 K（多單最新價低於當根開盤；空單反向對稱）永久收緊為回吐 10%，不等收線。
...
- 圖形量化採本次實作預設（非用戶逐項指定數值）：圖 3 連續 2 根已收線同向實體，每根至少通道寬 15%、總和至少平均通道寬 50%、相鄰重疊至多較小實體的 25%，收盤連續推進，且兩根均在開倉後（下一根即時銜接進場者保留其前一根突破 K 作為辨識背景）。圖 2 最近 6 根已收線方向效率至少 75%、收盤最大回撤至多最小通道寬 25%、中軌連續同向、收盘位於持倉側半通道；優先辨識圖 3，其餘有效 6 根資料歸圖 1。資料不足不新啟動保護，已啟動者保留。
