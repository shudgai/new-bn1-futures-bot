...
- 即時內軌單 K 例外：紅 K 必須從 KC 上軌外站回通道內，且進入通道的深度大於等於當前 KC 寬度的 `0.5` 倍，才可開空或將多單平倉轉空；綠 K 必須從下軌外站回通道內至少 `0.5` 倍，才可鏡像開多或將空單平倉轉多。不得將兩根 K 實體相加，當前 K 不必等收盤；CHOP_WAIT 與下單安全檢查仍保留。
