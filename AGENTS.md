...
- 即時內軌回轉例外：單根紅 K 從 KC 上軌外站回通道內至少當前 KC 寬度的 `0.5` 倍，可開空或將多單平倉轉空；若價格已進入通道，前一根已收盤紅 K 與目前即時紅 K 的實體都完整位於各自 KC 內，則兩根實體可相加，合計大於等於當前 KC 寬度的 `0.5` 倍同樣放行。綠 K 由下軌方向完全鏡像。當前 K 不必等收盤；CHOP_WAIT 與下單安全檢查仍保留。
- 對側外軌趨勢反轉：持空時若連續三根綠 K 上漲、現價位於 KC 上軌外，且最近兩根 MA3 位於上軌外並持續上升，必須先平空再開多；持多時以連續三根紅 K、現價與 MA3 位於下軌外且持續下降，鏡像先平多再開空。KC 外側追勢不套用通道寬度 `0.5` 的回通道門檻。若剛完成一次回轉反手，同一根即時 K 的現價隨即重新到達對側 KC 外軌，必須立刻平掉剛開的反向倉並同輪反手追單；不等收盤、不看 K 色、不再等待三根趨勢確認，且不受 CHOP_WAIT close-only 與同 K 反手去重限制。
