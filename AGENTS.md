...
- 一般平倉必須同時符合：本次持倉 MA3 曾出持倉側外軌後回到通道內、MA15 到持倉側外軌距離占通道寬度，以即時 K 計算嚴格不足 40%（用戶已撤回已收線比例的額外限制）、最近已收線 K 的收盤價嚴格過該根中軌（多單在下、空單在上），且即時價格仍在即時中軌的出場側；影線或單次 ticker 穿越不算收線確認。40% 邊界、價格僅碰中軌、MA3 仍在軌外、空間仍有 50%／60% 都不能單獨觸發一般平倉。
