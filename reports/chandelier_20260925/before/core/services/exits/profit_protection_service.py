...
def protection(position, price, fee, slippage, frame=None):
    ...
    reason = atr_exit_reason(position, price, frame)
    if not reason:
        return None
    return dict(triggered=True, reason=reason, locked_net=0.0,
                stop_price=position.get('atr_sl'))
...
class ProfitProtectionExitStrategy(IExitStrategy):
    ...
    def __init__(self, fee: float | None = None, slippage: float | None = None):
        from core.config import TAKER_FEE_RATE, SLIPPAGE_PCT
        self.fee = TAKER_FEE_RATE if fee is None else fee
        self.slippage = SLIPPAGE_PCT if slippage is None else slippage
    ...
    def evaluate_exit(
        ...
        **kwargs: Any
        ...
        if staged_enabled(position):
            return None
        from core.services.exits.entry_atr_protection import atr_exit_reason
        return atr_exit_reason(position, price, frame)
